...
then
    ...
    then
        ...
        sort $bname.bed+ | bedItemOverlapCount mm9 -bed12 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n > $bname.plus.bedGraph
        sort $bname.bed- | bedItemOverlapCount mm9 -bed12 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n | awk '{OFS="\t"; print $1,$2,$3,"-"$4}' > $bname.minus.bedGraph
    else
        echo "in bed6 format ..."
        sort $bname.bed+ | bedItemOverlapCount mm9 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n > $bname.plus.bedGraph
        sort $bname.bed- | bedItemOverlapCount mm9 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n | awk '{OFS="\t"; print $1,$2,$3,"-"$4}' > $bname.minus.bedGraph
        #sort $bname.bed+ | awk '{OFS="\t"; for(i=0;i<$6;i++) print $1, $2, $3, ".", 0, $4;}' | bedItemOverlapCount mm9 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n > $bname.plus.bedGraph
        #sort $bname.bed- | awk '{OFS="\t"; for(i=0;i<$6;i++) print $1, $2, $3, ".", 0, $4;}' | bedItemOverlapCount mm9 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n | awk '{OFS="\t"; print $1,$2,$3,"-"$4}' > $bname.minus.bedGraph
    fi
    echo "bedGraph2bw..."
...
then
    ...
    then
        echo "in bed12 format ..."
        sort $bname.bed | bedItemOverlapCount mm9 -bed12 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n > $bname.bedGraph
    else
        echo "in bed6 format ..."
        sort $bname.bed | bedItemOverlapCount mm9 -chromSize=$ANNOTATION/ChromInfo.txt stdin | sort -k1,1 -k2,2n > $bname.bedGraph
    fi
    echo "bedGraph2bw..."
